...
// Inside your OverallStats component:

export const OverallStats = ({ totalRamDetails,tabs }: any) => {
  // Use React's useMemo to only recalculate when the tabs array actually changes
  // console.log(`%ctotalRamDetails:${totalRamDetails.capacity},${totalRamDetails.availableCapacity}`,"color:red");
  
  const stats = useMemo(() => {
    return calculateDashboardStats(tabs || []);
  }, [tabs]);
  const totalCapacityGB = totalRamDetails ? totalRamDetails.capacity / 1024 : 0;
  const availableGB = totalRamDetails ? totalRamDetails.availableCapacity / 1024 : 0;
  const usedCapacityGB = totalCapacityGB - availableGB;
  const ramPercentage = totalCapacityGB > 0 
    ? Math.round((usedCapacityGB / totalCapacityGB) * 100) 
    : 0;
  console.log(`totalRamDetails:`,totalCapacityGB,availableGB,usedCapacityGB,ramPercentage);
  return (
    <div className="flex flex-col items-center justify-center w-full h-full p-2">
      
      {/* 1. Your Radial Gauge goes here */}
      <div className="RADIAL-GAUGE-CONTAINER mb-4">
        <RamRadialCharts ramPercentage={ramPercentage} totalCapacityGB={totalCapacityGB} usedCapacityGB={usedCapacityGB}/>
      </div>

      {/* 2. The Four Actionable Tiles */}
      <div className="grid grid-cols-4 gap-2 w-full">
        <StatItem label="Duplicate" value={stats.duplicates} color="text-yellow-500"/>
        <StatItem label="Sleeping" value={stats.sleeping} color="text-blue-500"/>
        <StatItem label="Heavy" value={stats.highRam} color="text-purple-500"/>
        <StatItem label="Threat" value={stats.highThreat} color="text-red-500"/>
      </div>
    </div>
  );
};
